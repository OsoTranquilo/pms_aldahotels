...
class DataBi(models.Model):
    ...
    def data_bi_get_codeine(self, reserva):
        ...
        import wdb
        wdb.set_trace()
        #reserva.reservation_id.partner_id.country_id.state_ids[0].ine_code
        # Debug Stop ------------------
        if reserva.reservation_id.partner_id.country_id.state_ids[0].ine_code:
            response = reserva.reservation_id.partner_id.country_id.state_ids[0].ine_code
            print(reserva.reservation_id.partner_id.name)
        else:
            for partner in reserva.reservation_id.checkin_partner_ids:
                print("Partner: ")
                if partner.partner_id.country_id.state_ids:
                    print("Ids: ")
                    response = partner.partner_id.country_id.state_ids[0].ine_code
                    print(response)
        return response
